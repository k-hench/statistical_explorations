...
-   [*posterior probability* ]{style="color:`r clr1`"}$P(H|D)$: how strongly we belief in our hypothesis given the data
-   [*likelyhood* ]{style="color:`r clr2`"}$P(D|H)$: the probability of data if the hypothesis were true
-   [*prior probability* ]{style="color:`r clr0`"}$P(H)$: how likely our hypothesis is in the first place

```{r, fig.align='center', echo = FALSE}
knitr::include_graphics("baeyes_theorem.svg")
```
...
ggplot(tibble(x = seq(mu - 4 * sigma, mu + 4 * sigma, length.out = 3)), aes(x)) +
 stat_function(fun = function(x){ dnorm(x, mu, sigma) },
               geom = "line", color = rgb(0,0,0,.3), n = 500) +
  stat_function(fun = function(x){ dnorm(x, mu, sigma) },
               xlim = c(mu - 3 * sigma, mu + 3 * sigma),
               geom = "area", color = clr2, fill = fll2, n = 500) +
  # stat_function(fun = function(x){ dnorm(x, mu, sigma) },
  #              xlim = c(mu + 2* sigma, mu + 3 * sigma),
  #              geom = "area", color = clr0, fill = fll0, n = 500) +
  stat_function(fun = function(x){ dnorm(x, mu, sigma) },
               xlim = c(mu - 2 * sigma, mu + 2 *sigma),
               geom = "area", color = clr2, fill = fll2, n = 500) +
  # stat_function(fun = function(x){ dnorm(x, mu, sigma) },
  #              xlim = c(mu + sigma, mu + (2 * sigma)),
  #              geom = "area", color = clr1, fill = fll1, n = 500) +
  stat_function(fun = function(x){ dnorm(x, mu, sigma) },
               xlim = c(mu - sigma, mu +  sigma),
               geom = "area", color = clr2, fill = fll2, n = 500) +
  geom_vline(xintercept = mu, linetype = 3, color = rgb(1,1,1,.5)) +
  geom_text(data = tibble(x = mu), aes(y = .075, label = "68%"),
            color = "white", family = "Josefin Sans") +
  geom_segment(data = tibble(x = mu),
               aes(xend = x + sigma, y = .025, yend = .025),
               color = "white") +
  geom_text(data = tibble(x = mu + .5 * sigma),
            aes(y = .015, label = "\U03C3"),
            color = "white") +
  labs(x = "p", y = "Density")
```
...
p_density <- ggplot(tibble(x = seq(0, 1, length.out = 3)), aes(x)) +
 stat_function(fun = function(x){ dbeta(x, shape1 = alpha, shape2 = beta) },
               xlim = c(bound_left, .009),
               geom = "area", color = clr1, fill = fll1, n = 500) +
  geom_vline(xintercept = mu, color = "white", linetype = 3) +
  geom_vline(xintercept = med, color = clr1, linetype = 3) +
  labs(x = "Measured Value", y = "Density") +
  theme(axis.title.x = element_blank(),
        axis.text.x = element_blank())

p_cummulative <- ggplot(tibble(x = seq(0, 1, length.out = 3)), aes(x)) +
 stat_function(fun = function(x){ pbeta(x, shape1 = alpha, shape2 = beta) },
               xlim = c(bound_left, .009),
               geom = "area", color = clr1, fill = fll1, n = 500) +
  geom_vline(xintercept = mu, color = "white", linetype = 3) +
  geom_vline(xintercept = med, color = clr1, linetype = 3) +
  labs(x = "Measured Value", y = "Cummulative Probability")

p_quantile <- ggplot(tibble(x = seq(0, 1, length.out = 3)), aes(x)) +
  geom_segment(data = tibble(x = c(0,0),
                             xend = c(.1,.9),
                             y = qbeta(c(.1,.9),
                             shape1 = alpha, shape2 = beta)),
                             aes(xend = xend, y = y, yend = y),
             linetype = 3, color = clr1) +
 stat_function(fun = function(x){ qbeta(x, shape1 = alpha, shape2 = beta) },
               xlim = c(0, 1),
               geom = "area", color = clr0, fill = fll0, n = 500) +
  stat_function(fun = function(x){ qbeta(x, shape1 = alpha, shape2 = beta) },
               xlim = c(0.1, 0.9),
               geom = "area", color = clr1, fill = fll1, n = 500) +
  geom_text(data = tibble(x = .5), aes(y = .0065, label = "90%"),
            color = "white", family = "Josefin Sans") +
  labs(x = "Cummulative probability", y = "Measured Value") +
  coord_cartesian(ylim = c(bound_left, .009))


(p_density / p_cummulative) +
  p_quantile + 
  plot_layout(design = "AAACC
                        BBBCC") +
  plot_annotation(tag_levels = "a",
                  caption = "**a.** PDF; **b.** CDF; **c.** Quatile Function") &
...
```{r, echo = FALSE}
tibble(`Posterior odds` = c("1 to 3", "3 to 20", "20 to 150", "> 150"),
       `Strength of evidence` = c("Interesting but not conclusive",
